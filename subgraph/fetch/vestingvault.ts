...
export function fetchVestingVault(address: Address): VestingVaultContract {
  ...
  let contract = VestingVaultContract.load(account.id);

  if (contract == null) {
    const endpoint = IVestingVault.bind(address);
    const token = endpoint.try_token();
    contract = new VestingVaultContract(account.id);
...
export function fetchVesting(contract: VestingVaultContract, beneficiary: Account, releaseTime: BigInt): Vesting {
  const id = contract.id.concat('/').concat(beneficiary.id).concat('/').concat(releaseTime.toString());
