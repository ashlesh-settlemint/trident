import { Address, ipfs, json, JSONValueKind, BigInt } from '@graphprotocol/graph-ts';
...
export function fetchStateMachine(address: Address): StateMachineMetadataContract {
  ...
  let contract = StateMachineMetadataContract.load(account.id);

  if (contract == null) {
    contract = new StateMachineMetadataContract(account.id);
  }
